...
let mediaType = ["imageMessage", "videoMessage", "stickerMessage", "audioMessage", "documentMessage", "documentWithCaptionMessage"]

module.exports = async (rem, messageRaw) => {
    ...
    message.isConsole = message.from.endsWith('@console') || message.from.endsWith('@console.group')
    message.isGroupMsg = message.from.endsWith("@g.us") || message.from.endsWith('@console.group')
    message.sender = message.isConsole ? message.from : message.isGroupMsg ? message.key.participant?.split(':')[0] : message.fromMe ? rem.user.id?.split('@')[0]?.split(':')[0] + '@s.whatsapp.net' : message.from?.split(':')[0]
    if(((message.sender && message.sender?.includes('@lid')) || (message?.key?.remoteJid?.includes('@lid') || message?.key?.participant?.includes('@lid'))) && (message?.key?.participantPn || message?.key?.senderPn)) {
        if(message.sender && message.sender?.includes('@lid')) message.sender = message?.key?.participantPn || message?.key?.senderPn
        if(message.key && message?.key?.remoteJid?.includes('@lid')) {
            message.key.senderLid = message?.key?.remoteJid
            message.key.remoteJid = message?.key?.participantPn || message?.key?.senderPn
        }
        if(message?.key?.participant && message?.key?.participant?.includes('@lid')) {
            message.key.participantLid = message?.key?.participant
            message.key.participant = message?.key?.participantPn || message?.key?.senderPn
        }
    }
    message.sender = message.isConsole ? message.from : message.sender?.split('@s.whatsapp.net')[0] + '@s.whatsapp.net'
    ...
    if((message.type == 'interactiveResponseMessage') && (message.message.interactiveResponseMessage?.nativeFlowResponseMessage?.name === 'menu_options') && message.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson) message.selectedRowId = JSON.parse(message.message.interactiveResponseMessage.nativeFlowResponseMessage.paramsJson).id
    if(message.isMedia) message.getMedia = async () => {
        const stream = await downloadContentFromMessage(message.message[message?.rawType || message?.type], (message.rawType || message.type).replace('Message', ''))
    ...
    if(message.quotedMsg) {
        ...
        if(type == 'ephemeralMessage') {
            message.quotedMsg = message.quotedMsg.ephemeralMessage.message
            type = Object.keys(message.quotedMsg)[0]
        }
        ...
        if(message.quotedMsg.isMedia) message.quotedMsg.getMedia = async () => {
            ...
            let quotedObj = message.quotedMsg
            if(!mime && message.quotedMsg?.message) {
                const type0 = Object.keys(message.quotedMsg.message)[0]
                if(mediaType.includes(type0)) {
                    mime = message.quotedMsg.message[type0].mimetype
                    quotedObj = message.quotedMsg.message[type0]
                }
            }
            let messageType = mime?.split('/')[0].replace('application', 'document') ? mime?.split('/')[0].replace('application', 'document') : mime?.split('/')[0]
            const stream = await downloadContentFromMessage(quotedObj, messageType)
            let buffer = Buffer.from([])
            for await(const chunk of stream) {
                buffer = Buffer.concat([buffer, chunk])
            }
    ...
    message.body = message.message.conversation || message.message[message.type].text || message.message[message.type].caption || message.message[message.type].selectedDisplayText || message.message[message.type]?.singleSelectReply?.selectedRowId || message.message[message.type].description || message.message[message.type].contentText //|| message.message[Object.keys(message.message)[0]].text || message.message[Object.keys(message.message)[1]].text
