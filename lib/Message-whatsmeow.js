function formatWhatsmeowMessage(rem, body, messageRaw, botInfo) {
    ...
    let formattedMessage = {
        ...
        t: new Date(info.Timestamp).getTime() / 1000,
        id: info.ID,
        from: info.Chat,
        fromMe: info.IsFromMe,
        chatId: info.Chat,
    ...
    if((info.Sender && info.Sender?.includes('@lid')) && info.SenderAlt) {
        if(formattedMessage.key && formattedMessage.key.remoteJid?.includes('@lid')) {
            formattedMessage.key.senderLid = formattedMessage.key.remoteJid
            formattedMessage.key.remoteJid = info.SenderAlt
        }
        if(formattedMessage.key.participant && formattedMessage.key.participant?.includes('@lid')) {
            formattedMessage.key.participantLid = formattedMessage.key.participant
            formattedMessage.key.participant = info.SenderAlt
        }
